...
class DepthMetric(BaseMetric):
    ...
    """
    METRICS = ('d0.5', 'd1', 'd2', 'd3', 'abs_rel', 'sq_rel', 'rmse', 'rmse_log',
               'log10', 'silog')
    ...
    def __init__(self,
        ...
        self.gt_depth_clamp = gt_depth_clamp
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            if self.depth_clamp:
                pred_label[pred_label > self.max_depth_eval] = self.max_depth_eval
                pred_label[pred_label < self.min_depth_eval] = self.min_depth_eval

            # format_only always for test dataset without ground truth
            if not self.format_only:
                gt_depth = data_sample['gt_depth_map']['data'].squeeze().to(
                    pred_label)
                if self.gt_depth_clamp:
                    gt_depth[gt_depth > self.max_depth_eval] = self.max_depth_eval
                    gt_depth[gt_depth < self.min_depth_eval] = self.min_depth_eval
            ...
            if self.output_dir is not None:
                basename = data_sample['img_path'].split('/')[-1].replace('.jpg', '.png')
    ...
    def _calc_all_metrics(gt_depth, pred_depth):
        ...
        d0_5 = torch.sum(thresh < 1.25**0.5).float() / len(thresh)
