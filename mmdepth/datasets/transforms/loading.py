...
from mmdepth.utils import datafrombytes
...
class LoadDepthAnnotation(BaseTransform):
    ...
    def __init__(self,
                 ...
                 to_float32: bool = True,
                 depthinpaint: bool = False,
                 depth_rescale_factor: float = 1.0,
                 backend_args: Optional[dict] = None) -> None:
        super().__init__()
        self.decode_backend = decode_backend
        self.to_float32 = to_float32
        self.depthinpaint = depthinpaint
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        if self.depthinpaint: # for sunrgbd
            depthVisData = gt_depth_map.astype(np.uint16)
...
@TRANSFORMS.register_module()
class LoadNormalAnnotation(BaseTransform):
    """Load ``normal_map`` annotation provided by depth estimation dataset.
    ...
    .. code-block:: python
        ...
        {
            'gt_normal_map': np.ndarray [Y, X, 3]
        }
    ...
    - gt_normal_map (np.ndarray): Depth map with shape (Y, X, 3) by
        default, and data type is float32 if set to_float32 = True.

    Args:
        decode_backend (str): The data decoding backend type. Options are
            'numpy', 'nifti', and 'cv2'. Defaults to 'cv2'.
        to_float32 (bool): Whether to convert the loaded normal map to a float32
            numpy array. If set to False, the loaded image is an uint16 array.
            Defaults to True.
        backend_args (dict, Optional): Arguments to instantiate a file backend.
            See :class:`mmengine.fileio` for details.
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        Returns:
            dict: The dict contains loaded depth map.
        """
        data_bytes = fileio.get(results['normal_map_path'], self.backend_args)
        gt_normal_map = datafrombytes(data_bytes, backend=self.decode_backend)

        if self.to_float32:
            gt_normal_map = gt_normal_map.astype(np.float32)

        gt_normal_map = gt_normal_map / 127.5 - 1
        gt_normal_map /= np.linalg.norm(gt_normal_map, ord=2, axis=2, keepdims=True)
        results['gt_normal_map'] = gt_normal_map
        results['seg_fields'].append('gt_normal_map')

        if self.valid_mask:
            mask_bytes = fileio.get(results['mask_map_path'], self.backend_args)
            gt_normal_mask = datafrombytes(mask_bytes, backend=self.decode_backend)
            gt_normal_mask = (gt_normal_mask != 0)
            results['gt_normal_mask'] = gt_normal_mask
            results['seg_fields'].append('gt_normal_mask')        
