...
from mmdepth.datasets.dataset_wrappers import MultiImageMixDataset
...
class KBCrop(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.depth:
            depth_cropped = results["gt_depth_map"][top_margin:top_margin +
                                                self.height,
                                                left_margin:left_margin +
                                                self.width]
            results["gt_depth_map"] = depth_cropped
            results["depth_shape"] = results["gt_depth_map"].shape

        img_cropped = results["img"][top_margin:top_margin + self.height,
                                     left_margin:left_margin + self.width, :]
        results["img"] = img_cropped
        results["ori_shape"] = img_cropped.shape[:2]

        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(depth={self.depth}, ' \
