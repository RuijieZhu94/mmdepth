...
# def compute_seg(aligned_norm, D):
#         """
#         inputs:
#             rgb                 b, 3, H, W
...
#         segment = torch.stack([torch.from_numpy(felz_seg(normalize(cost_down_np[i]), normalize(cost_right_np[i]), 0, 0, h, w, scale=1, min_size=50)).cuda() for i in range(b)])
#         segment += 1
...
#         area = torch.zeros((b, max_num)).cuda()
#         area.scatter_add_(1, segment.view(b, -1), torch.ones((b, 1, h, w)).view(b, -1).cuda())
